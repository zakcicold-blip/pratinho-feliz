...
import NovoLinkForm from "../links/NovoLinkForm";
import DesligarLinkButton from "../links/DesligarLinkButton";
 ...
 */
export default async function Links({ parceira, base, somenteLeitura = false }: PropsVisao) {
  const links = await db.linkParceira.findMany({
      ...
      {!somenteLeitura && (
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">
            Crie um link para cada lugar onde você divulga. Assim dá para ver qual peça trouxe
            gente de verdade, em vez de adivinhar.
          </p>
        </div>
      )}

      {!somenteLeitura && parceira.ativa && <NovoLinkForm />}
        ...
        {ativos.map((l) => (
          <div key={l.id} className="rounded-2xl border border-stone-200/70 bg-white p-5 shadow-card">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              {!somenteLeitura && <DesligarLinkButton linkId={l.id} rotulo={l.rotulo} />}
            </div>
