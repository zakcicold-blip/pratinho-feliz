...
import ChavePixForm from "../pagamentos/ChavePixForm";
...
export default async function Extrato({ parceira, somenteLeitura = false }: PropsVisao) {
  ...
  return (
    <div className="space-y-6">
      ...
      {somenteLeitura ? (
        <p className="rounded-2xl border border-stone-200/70 bg-white p-5 text-sm text-stone-500 shadow-card">
          Chave PIX cadastrada:{" "}
          {chaveAtual?.chavePix ? (
            <code className="text-stone-700">{chaveAtual.chavePix}</code>
          ) : (
            "nenhuma ainda"
          )}
        </p>
