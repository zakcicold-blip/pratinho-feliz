...
/**
 * As quatro perguntas que a parceira faz toda semana, nessa ordem: quantas
 * pessoas clicaram, quantas criaram conta, quantas estao pagando, quanto ela
 * recebe este mes. O resto e detalhe e mora nas outras abas.
 */
export default async function Resumo({ parceira, base, somenteLeitura = false }: PropsVisao) {
  const mes = mesDe(new Date());

  const [resumo, primeiroLink] = await Promise.all([
