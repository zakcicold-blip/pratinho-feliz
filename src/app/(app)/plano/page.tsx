    ...
    <>
      ...
      <div className="px-4 py-4">
        <div className="mb-4 flex gap-1 rounded-2xl bg-stone-100 p-1 text-sm">
          ...
          <Link
            href={`/plano?view=30dias&dia=${diaClamped}`}
            className={cn(
              "flex-1 rounded-xl py-1.5 text-center font-semibold transition",
              view === "30dias" ? "bg-white text-stone-800 shadow-card" : "text-stone-500"
            )}
          >
            30 dias
          </Link>
        ...
        {view === "30dias" ? (
          <div className="grid grid-cols-6 gap-2">
            {Array.from({ length: 30 }, (_, i) => i).map((i) => {
              ...
              const feedbacksPositivos = slotsDia.filter(
                (s) => s.feedback && (s.feedback.estado === "GOSTOU" || s.feedback.estado === "ACEITOU")
              ).length;
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    ativo
                      ? "border-orange-400 bg-orange-50 shadow-card"
                      : "border-stone-200/70 bg-white hover:border-stone-300"
                  )}
                >
                  <span className="text-[10px] text-stone-400">{i + 1}</span>
                  <span className="text-xs font-semibold text-stone-700">{formatDiaMes(data)}</span>
                  {feedbacksPositivos > 0 && (
                    <Heart size={10} className="mt-0.5 text-emerald-500" fill="currentColor" />
                  )}
                </Link>
              );
            })}
